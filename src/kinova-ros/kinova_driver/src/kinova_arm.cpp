...
//    tf_prefix_ = kinova_robotType_ + "_" + boost::lexical_cast<string>(same_type_index); // in case of multiple same_type robots
    ...
    restarting_service_ = node_handle_.advertiseService("in/restart_api", &KinovaArm::restartAPIServiceCallback, this);
...
//By MOSTAFA
bool KinovaArm::restartAPIServiceCallback(kinova_msgs::HomeArm::Request &req, kinova_msgs::HomeArm::Response &res)
{
    kinova_comm_.restartAPI();
    kinova_comm_.initFingers();
    res.homearm_result = "KINOVA ARM API RESTARTED";
    return true;
}
...
void KinovaArm::fingerVelocityCallback(const kinova_msgs::FingerPositionConstPtr& finger_vel)
{
    if (!kinova_comm_.isStopped())
    {
        ...
        //kinova_comm_.setFingerVelocities(finger_velocities_);
        kinova_comm_.setFingerPositions(finger_velocities_);
        //ROS_INFO("finger velocity call back is called");
    }
}
