...
class MoveitApp():
    ...
    def robot_goto(self, bowlTrue=True):
        if (bowlTrue):
            self.move_to_pub.publish("bowl_search_pos")
        else:
            self.move_to_pub.publish("face_search_pos")
