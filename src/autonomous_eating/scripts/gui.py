...
class GUIApp():
  ...
  def gui_mode_callback(self, data):
    self.selectness = data.selectness
    #self.master.after(1,self.updateAcitvationBar(data.selectness))
    if data.y < 15:
      self.itciY = data.y/20.0*600.0
    else:
      self.itciY = (data.y-10.0)/20.0*600.0

    self.itciX = data.x/10*350
  ...
  def updateItciImg(self): #frame, x=25,y=25):
    ...
    self.circleSize = 25
    self.x1 = self.itciX - self.circleSize
    self.x2 = self.itciX + self.circleSize
    self.y1 = self.itciY - self.circleSize
    self.y2 = self.itciY + self.circleSize
    self.itciCanvas.create_oval(self.x1,self.y1,self.x2,self.y2, width=5, outline="red")
    self.activationBar['value'] = self.selectness
  ...
  def __init__(self, master):
    ...
    self.itciY = 0
    self.itciX = 0
    self.status = ("Connected", "Disconnected", "OK", "False")
    self.selectness = 0
